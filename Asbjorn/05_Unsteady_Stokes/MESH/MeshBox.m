...
if distribution==0 || distribution==1
    nglob = (NELX*(Npoints-1)+1)*(NELY*(Npoints-1)+1);	% number of global nodes
elseif distribution==2
    nglob = (NELX*(Npoints))*(NELY*(Npoints));	% number of global nodes
end
...
for ey=1:NELY,
    for ex=1:NELX,
        ...
        elseif distribution==2
            ig = reshape([1:Npoints*Npoints],Npoints,Npoints)+last_iglob;
        end
        iglob(:,:,e) = ig;
