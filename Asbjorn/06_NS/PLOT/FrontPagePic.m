...
for dt = dt_vect
    ...
    for polOrder = polOrders
        ...
        % plotElements(mesh,study.ObjectCoords)
        % plotMesh2D(mesh)
        ...
        node = 82;
        u1 = zeros(length(mesh.X),1); 
        u1(node) = 1; 
        u2 = u1;
            
[data_interp] = twoD_element_interpolator(1001, 1, u1, u2, zeros(length(mesh.Xp), 1), mesh.IX, mesh.IXp, mesh.X, mesh.Xp, study.N);
figure()
[X, Y] = meshgrid(unique(data_interp(1, :, 2)), unique(data_interp(1, :, 2)));
Z = reshape(data_interp(1, :, 3), size(X));
h = surf(X, Y, Z, 'EdgeColor', 'none');

% Define the custom color map
n = 256; % Total number of points in the colormap
switch_at = round(7 * n / 10);
% Blue to white
white = 0.8;
blue_to_white = [linspace(0, white, n - switch_at)', linspace(0, white, n - switch_at)', ones(n - switch_at, 1) * white];

% White to red
white_to_orangy_red = [linspace(white, 0.7, switch_at)', linspace(white, 0, switch_at)', linspace(white, 0, switch_at)'];

% Combine the two colormaps
paraview_cmap = [blue_to_white; white_to_orangy_red];

% Apply the custom colormap
colormap(paraview_cmap);

temp = round(unique(mesh.X(:, 2)) * (size(Z, 1) - 1)) + 1;
hold on
plot3(X(:, temp), Y(:, temp), Z(:, temp), 'k');
plot3(X(temp, :)', Y(temp, :)', Z(temp, :)', 'k');
axis off

% Set the 3D view
azimuth = -40; % Horizontal rotation (degrees)
elevation = 40; % Vertical rotation (degrees)
...
% Export the figure as a PNG with transparent background
exportgraphics(gcf, 'Figures\frontpagePIC.png', 'BackgroundColor', 'none', 'Resolution', 300); % Filename: surface_plot.png, 300 dpi resolution
...
function folderName = createSimulationFolder(mesh, study, dt, NELX, NELY)
    ...
    if strcmp(study.convergence, 'spatial')
        folderName = fullfile('simulations3', sprintf('Vortex_%s_%dx%d_dt%s', fem_type, NX, NY, dt_str));
