...
for dt = dt_vect
    ...
    polOrders = 10; % Default for temporal
    if strcmp(study.convergence, 'spatial')
        polOrders = [3:5];
    end
    ...
    for polOrder = polOrders
        ...
        study.folderName = createSimulationFolder(mesh,study, dt, NELX, NELY)
...
function folderName = createSimulationFolder(mesh, study, dt, NELX, NELY)
    ...
    if strcmp(study.convergence, 'spatial')
        folderName = fullfile('simulationsTEST', sprintf('Vortex_%s_%dx%d_dt%s', fem_type, NX, NY, dt_str));
