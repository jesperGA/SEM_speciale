function plotSol2D(mesh,u1,u2,study,t)
    ...
    nodes=mesh.X(:,1);
    quiver(  mesh.X(nodes,2),   mesh.X(nodes,3), u1(nodes), u2(nodes), scale_factor, 'Color', defaultColors(1,:),'LineWidth',1.5)
    quiver(  mesh.X(nodes,2),   mesh.X(nodes,3), study.U1(mesh.X(:,2),mesh.X(:,3),t), study.U2(mesh.X(:,2),mesh.X(:,3),t), scale_factor, 'Color', defaultColors(2,:))
