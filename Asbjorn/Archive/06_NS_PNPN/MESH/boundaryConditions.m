...
%                             Boundary Conditions                         %
%-------------------------------------------------------------------------%
mesh.bound_u1 = zeros((4*(NELX*N+1)-4),3);
mesh.bound_u2 = zeros((4*(NELX*N+1)-4),3);

nodes = getBoundaryNodes(mesh);
mesh.bound_u1(:,1) = nodes;
mesh.bound_u2(:,1) = nodes;

if strcmp(study.example,'Roenquist_NS')
    mesh.bound_u1(:,3) = U1(mesh.X(nodes,2),mesh.X(nodes,3),0);
    mesh.bound_u2(:,3) = U2(mesh.X(nodes,2),mesh.X(nodes,3),0);
else
    mesh.bound_u1(:,3) = U1(mesh.X(nodes,2),mesh.X(nodes,3));
    mesh.bound_u2(:,3) = U2(mesh.X(nodes,2),mesh.X(nodes,3));
end
...
function nodes = getBoundaryNodes(mesh)
    % South
