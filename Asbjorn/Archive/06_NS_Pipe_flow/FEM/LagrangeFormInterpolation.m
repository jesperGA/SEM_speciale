function fit = LagrangeFormInterpolation(knots,data,t)
...
%   data=[d0 d1 ... dn]   is a row of the corresponding n+1 values
...
m=length(knots);
if (m~=length(data) ) 
    disp('Der skal være lige mange knuder og data'); 
    fit=NaN; return; 
end
...
temp=repmat(data',1,length(t));
fit=sum(cardinals.*temp);
