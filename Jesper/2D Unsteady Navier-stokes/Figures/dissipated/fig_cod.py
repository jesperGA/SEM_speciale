...
main_directory = 'C:\\Users\\jga91\\OneDrive - Danmarks Tekniske Universitet\\SEM Speciale\\Kode\\SEM_speciale\\Jesper\\2D Unsteady Navier-stokes\\Figures to stability comp\\'
...
for subdir, dirs, files in os.walk(main_directory):
    ...
    if nek5000_file:
        ...
        file = os.path.join(base_path, nek5000_file)
        name = nek5000_file.split('.')[0]
        RE = float(name.split('RE')[1])



        # create a new 'Nek5000 Reader'
        domain = Nek5000Reader(registrationName=name, FileName=file)
        ...
        if RE<=200:
            ...
            calculator1.Function = 'sqrt((coordsX-0.8)^2 + (coordsY-0.2)^2)'
        ...
        ColorBy(threshold1Display,('POINTS', None))
