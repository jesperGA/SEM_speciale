function mesh = modify_to_Backstep(h,L,mesh,V0,element_to_remove,fully)
...
top_bc = mesh.Xv(:,2) <= 0.5 & mesh.Xv(:,3) == 0.5;
east_bc = mesh.Xv(:,2) == 0.5 & mesh.Xv(:,3) <= 0.5;
box_BC = top_bc | east_bc;
...
elseif fully
    umax = 3/2*V0;
    R = h/4; r = linspace(0,2*R,numel(west_bound));
    U = umax.*(1-((r-R)/R).^2);
