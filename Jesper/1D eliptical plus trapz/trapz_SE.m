function [me,ke] = trapz_SE(x,E,dens,me_Gauss,w,xi)
...
for i = 1:N+1
    for j = 1:N+1
        ...
        for k = 1:N+1
            ...
            if k ==1
                dl1 = dlagrange(xi,N,j,k); dl2 = dlagrange(xi,N,i,k);a = exp(x(1)+(xi(k)+1)*J);
                old_val = dl1*dl2*a;
            else
                %Compute sum
                dl1 = dlagrange(xi,N,j,k); dl2 = dlagrange(xi,N,i,k);a = exp(x(1)+(xi(k)+1)*J);
                temp = temp +(xi(k)-xi(k-1))*iJ*(dl1*dl2*exp(x(1)+(xi(k)+1)*J)+old_val)/2;
                old_val = dl1*dl2*a;
            end
        end
        % ke(i,j) =  exp(x(i))*temp;
