...
# Plotting the data
plt.semilogy(Pn[:,0], Pn[:,1], '-o', label=r'$u \in \mathbb{P}_N-\mathbb{P}_N$', linewidth=2, markersize=10, color=DC[1])
plt.semilogy(Pn[:,0], Pn[:,2], '-o', label=r'$p \in \mathbb{P}_N-\mathbb{P}_N$', linewidth=2, markersize=10, color=DC[3])
plt.semilogy(Pn2[:,0], Pn2[:,1], '-o', label=r'$u \in \mathbb{P}_N-\mathbb{P}_{N-2}$', linewidth=2, markersize=10, color=DC[0])
...
# Enhance minor ticks for logarithmic scale
ax.yaxis.set_minor_locator(ticker.LogLocator(subs='all'))

# Turn on minor ticks and minor grid, keeping them logarithmic
ax.minorticks_on()
