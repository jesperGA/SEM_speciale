function opt = Assembly(mesh, study, opt)
    ...
    %         Includes opt.A, opt.B, opt.C, opt.Null, opt.P
    ...
    parfor e = 1:numElems
        ...
        [a(:,:,e), b(:,:,e), grad1(:,:,e), grad2(:,:,e), J(:,:,e)] = elementMatrix2D(xy, w, D, N);

        % Calculate coupling matrices
        [d1(:,:,e), d2(:,:,e)] = calculateDMatrices(xy, N, xi, xi_gl, w_gl);
    end

    % Store results in opt struct
    opt.b = b;
    opt.grad1 = grad1;
    opt.grad2 = grad2;
    opt.J = J;
...
function [Null, g] = createSupportMatrix(bound, neqn)
    % createSupportMatrix: Creates support matrix for boundary conditions
    % Inputs:
    %   bound - Boundary condition matrix
    %   neqn - Number of equations
    % Outputs:
    %   Null - Null matrix for boundary conditions
    %   g - Boundary condition vector
...
function [A_global, B_global] = assembleGlobalMatrices(edof1, edof2, ae, be, ldof1, ldof2, numElems, neqn1, neqn2)
    ...
    % Inputs:
    %   edof1 - Element degrees of freedom for matrix A
    %   edof2 - Element degrees of freedom for matrix B
    %   ae - Element matrix A
    %   be - Element matrix B
    %   ldof1 - Number of local degrees of freedom for matrix A
    %   ldof2 - Number of local degrees of freedom for matrix B
    %   numElems - Number of elements
    %   neqn1 - Number of equations for matrix A
    %   neqn2 - Number of equations for matrix B
    % Outputs:
    %   A_global - Assembled global matrix A
    %   B_global - Assembled global matrix B
